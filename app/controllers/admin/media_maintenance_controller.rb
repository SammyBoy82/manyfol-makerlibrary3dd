...
module Admin
  class MediaMaintenanceController < ApplicationController
    ...
    def index
      ...
      libraries =
        Library.order(:name).to_a

      online_libraries, @offline_libraries =
        libraries.partition(&:storage_exists?)

      @health =
        online_libraries.map do |library|
    ...
    def missing_sources
      ...
      return unless ensure_storage_online!(@library)
    ...
    def ensure_storage_online!(library)
      return true if library.storage_exists?

      redirect_to(
        admin_media_maintenance_path,
        alert: "Storage for #{library.name} is currently offline. No maintenance action was queued."
      )
