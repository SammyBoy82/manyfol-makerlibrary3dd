module PreviewEnrichment
  module Providers
    class ThingiverseSearch < RepositorySearchBase
      private
      ...
      def search_repository(query)
        uri = URI("https://www.thingiverse.com/search")
      ...
      def extract_results(html, search_url, query)
        ...
        html.scan(
          /<a[^>]+href=["']([^"']*\/thing:[0-9]+[^"']*)["'][^>]*>(.*?)<\/a>/im
        ).each do |href, content|
          title = normalize(content)

          next if title.blank?

          url = URI.join("https://www.thingiverse.com", href).to_s
