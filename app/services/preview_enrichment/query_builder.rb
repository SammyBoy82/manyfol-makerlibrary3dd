module PreviewEnrichment
  class QueryBuilder
    ...
    def initialize(model)
      @model = model
      @context = SearchContext.new(model)
    end

    def queries
      return [] unless context.searchable?
    ...
    def evidence
      {
        model_id: model.id,
        name: model.name,
        effective_search_name: context.search_name,
        ...
        search_context: context.evidence
      }
    end
    ...
    def primary_query
      %("#{clean(context.search_name)}" 3D printable model)
    end

    def stl_query
      %("#{clean(context.search_name)}" STL)
    end

    def creator_query
      return if model.creator&.name.blank?

      %("#{clean(context.search_name)}" "#{clean(model.creator.name)}")
    end
